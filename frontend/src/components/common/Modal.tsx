...
const Modal = forwardRef<ModalRef, ModalProps>(({ children, width = "w-96", isVisible: isVisibleProp, onClose }, ref) => {
  const [internalVisible, setInternalVisible] = useState(false);

  // Use controlled or uncontrolled visibility
  const isVisible = typeof isVisibleProp === "boolean" ? isVisibleProp : internalVisible;
  ...
  return (
    <div className="fixed inset-0 backdrop-blur-sm bg-opacity-50 flex justify-center items-center z-50" onClick={handleClose}>
      <div className={`relative bg-white p-6 rounded-lg shadow-lg ${width}`}>
        <button className="absolute top-2 right-2 text-gray-400" onClick={onClose ? onClose : () => setInternalVisible(false)}>
          <FaTimes />
        </button>
        {children}
      </div>
